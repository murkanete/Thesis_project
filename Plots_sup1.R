...
# multinomial condition, variance and bias condition for superpopulation 1.
...
sampling_distribution_sup1$Domain = ifelse(sampling_distribution_sup1$Domain == "A", "Man",  # label A and B in domain_random Male and Female for convenience
                                           ifelse(sampling_distribution_sup1$Domain == "B", "Vrouw",
                                                  sampling_distribution_sup1$Domain))
sampling_distribution_sup1$Domain = as.factor(sampling_distribution_sup1$Domain)
levels(sampling_distribution_sup1$Domain) = c("Male", "Female")

model_distribution_sup1$Domain = ifelse(model_distribution_sup1$Domain == "A", "Man", 
                                           ifelse(model_distribution_sup1$Domain == "B", "Vrouw",
                                                  model_distribution_sup1$Domain))
model_distribution_sup1$Domain = as.factor(model_distribution_sup1$Domain)
levels(model_distribution_sup1$Domain) = c("Male", "Female")

joint_distribution_sup1$Domain = ifelse(joint_distribution_sup1$Domain == "A", "Man", 
                                           ifelse(joint_distribution_sup1$Domain == "B", "Vrouw",
                                                  joint_distribution_sup1$Domain))
joint_distribution_sup1$Domain = as.factor(joint_distribution_sup1$Domain)
levels(joint_distribution_sup1$Domain) = c("Male", "Female")

#############################################################################################

# Benchmark estimators

CV_lineplot = joint_distribution_sup1 %>% 
  filter(Estimate %in% c("RRMSE_joint", "CV_joint", "RB_joint")) %>%
  mutate(Estimate_perc = abs(joint*100))

CV_lineplot_samp = model_distribution_sup1 %>% 
...
boxplot = sampling_distribution_sup1 %>%
...
benchmark_MSE = model_distribution_sup1 %>%
...
ggplot(boxplot, aes(x = Estimate, y = Estimate_perc, fill = Estimate)) +
  ...
  scale_y_continuous(limits = c(1.65, 2.2)) +  
  labs(y = "CSRM (%)", x = NULL) +
  theme_minimal() +
  theme(
...
boxplot = model_distribution_sup1 %>%
  filter(Domain == "Male" &                                           # Male
           Condition %in% c("Baseline") &
           Estimate %in% c("CV_Vdesign", "RRMSE_GMSE", "RRMSE_PEM")) %>% mutate(Estimate_perc = Wo*100)
...
benchmark_MSE = joint_distribution_sup1 %>%
...
benchmark_bias = model_distribution_sup1 %>%
...
benchmark_bias = joint_distribution_sup1 %>%
...
sampling_distribution_sup1_c3$Domain = as.factor(sampling_distribution_sup1_c3$Domain)
levels(sampling_distribution_sup1_c3$Domain) = c("Male", "Female")

c3_sup1 = sampling_distribution_sup1_c3 %>%
...
c3_sup1$Estimate = as.factor(c3_sup1$Estimate)
c3_sup1$Estimate = droplevels(c3_sup1$Estimate)
c3_sup1$Estimate = factor(c3_sup1$Estimate, levels = c("CV_Vdesign", "RRMSE_GMSE", "RRMSE_PEM"))


c3_sup1_long <- c3_sup1 %>%
  pivot_longer(
...
c3_sup1_long$Category = as.factor(c3_sup1_long$Category)
c3_sup1_long$Category = factor(c3_sup1_long$Category,
                               levels = c("Low",
                                          "Middle",
                                          "High"))
...
label_data <- data.frame(
  ...
  y = rep(c(1, 1, 3.5), each = 3),
  label = rep(c("Vdesign  GMSE  PEM (MSE)"), times = 3),
  Domain = rep(c("Male", "Female"), each = 9)  
)

label_data[label_data$Category == "Low" & label_data$Domain == "Female", "y"] = 4.2
label_data[label_data$Category == "High" & label_data$Domain == "Female", "y"] = 1
label_data$Domain = as.factor(label_data$Domain)
label_data$Domain = factor(label_data$Domain,
                           levels = c("Male", "Female"))

ggplot(c3_sup1_long, aes(x = Category, y = Estimate_perc, fill = fill_group)) +
  ...
  geom_segment(
    data = data.frame(
      ...
      y = c(1.98, 5.55, 1.51, 1.86, 4.16, 1.60),
      yend = c(1.98, 5.55, 1.51, 1.86, 4.16, 1.60),
...
ggplot(c3_sup1_long, aes(x = Category, y = Estimate_perc, fill = Domain)) +
  geom_boxplot(position = position_dodge(width = 0.8)) +
  facet_wrap(~ Domain)  +
  geom_segment(
    data = data.frame(
...
overparameterization_plot = ggplot(boxplot, aes(x = Estimate, y = Estimate_perc, fill = Estimate)) + 
  ...
  labs(y = NULL, x = NULL) +
  theme_minimal() +
  scale_y_continuous(limits = c(1.6, 2.25)) +
  theme(
    axis.title.y = element_text(size = 16),   
    axis.text = element_text(size = 14),   
    strip.text = element_text(size = 16),  
...
benchmark_var = model_distribution_sup1 %>%
...
benchmark_var = joint_distribution_sup1 %>%
