...
N_large = nrow(pop_balanced) # N = 122 250; nrow(pop_balanced)
n_large = round(0.05*N)  # n = 6112
# non-informative sampling (SRSWOR)
pop_balanced$pi = n_large/N_large 
pop_balanced$id = 1:nrow(pop_balanced)

#create synthetic income and region variables that are strongly related to the target variable
assign_income <- function(edu) {
  if (edu == "Wo") {
    sample(c("Low", "Middle", "High"), size = 1, prob = c(0.1, 0.2, 0.7))  
  } else {
    sample(c("Low", "Middle", "High"), size = 1, prob = c(0.7, 0.2, 0.1))  
  }
}

pop_balanced$inkomen3 <- sapply(pop_balanced$onderwijs, assign_income)

assign_region <- function(edu) {
  if (edu == "Wo") {
    sample(c("North", "West", "South", "East"), size = 1, prob = c(0.15, 0.7, 0.1, 0.05))  
  } else {
    sample(c("North", "West", "South", "East"), size = 1, prob = c(0.05, 0.1, 0.7, 0.15))  
  }
}

pop_balanced$region <- sapply(pop_balanced$onderwijs, assign_region)


pop_balanced$inkomen3 = as.factor(pop_balanced$inkomen3)
pop_balanced$region = as.factor(pop_balanced$region)
...
covs_2 = c("geslacht", "leeftijd", "inkomen3", "region", "I(leeftijd**2)") # geslacht will be the domain variable
formula_2 = as.formula(paste("onderwijs ~", paste(covs_2, collapse = " + "))) 
superpopulation_2 = glm(formula_2, data = pop_balanced, family='binomial')
...
for (B in 1:B_pop) {
  ...
  p.true = predict(superpopulation_2, newdata = sim_register, type="response")
  ...
  for (b in 1:b_samp) { # sampling variability
    ...
    imp_model = glm(formula_2, data = samp, family='binomial') # parameter variability
...
save(P_model, P_sampling, file = "sup2_baseline_P.RData")
write.csv(results$results_sampling_df, "sup2_baseline_sampling_distribution.csv")
write.csv(results$results_model_df, "sup2_baseline_model_distribution.csv")
write.csv(results$results_joint_df, "sup2_baseline_joint_distribution.csv")
...
# write results to disk
save(P_model, P_sampling, file = "sup2_small_sample_P.RData")
write.csv(results$results_sampling_df, "sup2_small_sample_sampling_distribution.csv")
write.csv(results$results_model_df, "sup2_small_sample_model_distribution.csv")
write.csv(results$results_joint_df, "sup2_small_sample_joint_distribution.csv")
...
N_small = nrow(pop_balanced_small) # N = 122 250; nrow(pop_balanced)
n_large_2 = round(0.05*N_small)  # n = 6112
# non-informative sampling (SRSWOR)
...
pop_balanced_small$inkomen3 <- sapply(pop_balanced_small$onderwijs, assign_income)
...
pop_balanced_small$region <- sapply(pop_balanced_small$onderwijs, assign_region)


pop_balanced_small$inkomen3 = as.factor(pop_balanced_small$inkomen3)
pop_balanced_small$region = as.factor(pop_balanced_small$region)

# create the superpopulation
# log-scale the numerical variable
pop_balanced_small$leeftijd = scale(log(pop_balanced_small$leeftijd+0.0001)) 

superpopulation_2_smallpop = glm(formula_2, data = pop_balanced_small, family='binomial')
...
for (B in 1:B_pop) {
  ...
  p.true = predict(superpopulation_2_smallpop, newdata = sim_register, type="response")
  sim_register$onderwijs = factor(sapply(p.true, function(p) rbinom(1, 1, p)),
                                  labels = c("Geen wo", "Wo"))
...
# write results to disk
save(P_model, P_sampling, file = "sup2_small_population_P.RData")
write.csv(results$results_sampling_df, "sup2_small_population_sampling_distribution.csv")
write.csv(results$results_model_df, "sup2_small_population_model_distribution.csv")
write.csv(results$results_joint_df, "sup2_small_population_joint_distribution.csv")
...
covs_imp = c("geslacht", "leeftijd", "inkomen3", "region", "I(leeftijd**2)", "gemeente", "random")
...
# write results to disk
save(P_model, P_sampling, file = "sup2_overparameterisation_P.RData")
write.csv(results$results_sampling_df, "sup2_overparameterisation_sampling_distribution.csv")
write.csv(results$results_model_df, "sup2_overparameterisation_model_distribution.csv")
write.csv(results$results_joint_df, "sup2_overparameterisation_joint_distribution.csv")
...
##########################################################################################
set.seed(4789)
Nh = summary(pop_balanced$region) # oversample people with middle and higher incomes (minority groups)

nh = c("East" = round(0.4*n),
       "North" = round(0.4*n),
       "South" = round(0.1*n),
       "West" = round(0.1*n))
pi = nh/Nh
pop_balanced$pi = ifelse(pop_balanced$region == "East", as.numeric(pi["East"]),
                         ifelse(pop_balanced$region == "North", as.numeric(pi["North"]),
                                ifelse(pop_balanced$region == "South", as.numeric(pi["South"]),
                                       as.numeric(pi["West"]))))
...
for (B in 1:B_pop) {
  ...
  for (b in 1:b_samp) { # sampling variability
    ...
    for (stratum in seq_along(Nh)) {
      strat_levels = levels(sim_register$region)
      sim_subset = sim_register[sim_register$region == strat_levels[stratum], ]
    ...
    # train an imputation model
    imp_model = glm(formula_2, data = samp, family='binomial') # input for GMSE
    ...
    # train an imputation model
    imp_model = glm(formula_2, data = samp, family='binomial', weights = 1/samp$pi) # input for Vdesign and MSEboot
...
# write results to disk
save(P_model, P_sampling, file = "sup2_nonignorable_P.RData")
write.csv(results$results_sampling_df, "sup2_nonignorable_sampling_distribution.csv")
write.csv(results$results_model_df, "sup2_nonignorable_model_distribution.csv")
write.csv(results$results_joint_df, "sup2_nonignorable_joint_distribution.csv")
...
# write results to disk
save(P_model, P_sampling, file = "sup2_designbias_P.RData")
write.csv(results$results_sampling_df, "sup2_designbias_sampling_distribution.csv")
write.csv(results$results_model_df, "sup2_designbias_model_distribution.csv")
write.csv(results$results_joint_df, "sup2_designbias_joint_distribution.csv")
...
covs_imp = c("leeftijd", "inkomen3", "region", "I(leeftijd**2)")
formula_imp = as.formula(paste("onderwijs ~", paste(covs_imp, collapse = " + "))) 
...
# write results to disk
save(P_model, P_sampling, file = "sup2_modelbias_small_P.RData")
write.csv(results$results_sampling_df, "sup2_modelbias_small_sampling_distribution.csv")
write.csv(results$results_model_df, "sup2_modelbias_small_model_distribution.csv")
write.csv(results$results_joint_df, "sup2_modelbias_small_joint_distribution.csv")
...
covs_syn = c("geslacht_syn", "leeftijd", "inkomen3", "region", "I(leeftijd**2)") # geslacht will be the domain variable
...
# write results to disk
save(P_model, P_sampling, file = "sup2_modelbias_large_P.RData")
write.csv(results$results_sampling_df, "sup2_modelbias_large_sampling_distribution.csv")
write.csv(results$results_model_df, "sup2_modelbias_large_model_distribution.csv")
write.csv(results$results_joint_df, "sup2_modelbias_large_joint_distribution.csv")
...
# write results to disk
save(P_model, P_sampling, file = "sup2_modelbias_large_small_samp_P.RData")
write.csv(results$results_sampling_df, "sup2_modelbias_large_small_samp_sampling_distribution.csv")
write.csv(results$results_model_df, "sup2_modelbias_large_small_samp_model_distribution.csv")
write.csv(results$results_joint_df, "sup2_modelbias_large_small_samp_joint_distribution.csv")
...
assign_income <- function(edu) {
  if (edu == "High") {
    sample(c("Low", "Middle", "High"), size = 1, prob = c(0.05, 0.1, 0.85))  
  } else if (edu == "Middle") {
    sample(c("Low", "Middle", "High"), size = 1, prob = c(0.2, 0.3, 0.5))  
  } else {
    sample(c("Low", "Middle", "High"), size = 1, prob = c(0.85, 0.1, 0.05))
  }
}

pop$inkomen3_syn <- sapply(pop$onderwijs, assign_income)

assign_region <- function(edu) {
  if (edu == "High") {
    sample(c("North", "West", "South", "East"), size = 1, prob = c(0.15, 0.7, 0.1, 0.05))  
  } else if (edu == "Middle") {
    sample(c("North", "West", "South", "East"), size = 1, prob = c(0.1, 0.6, 0.2, 0.1))  
  } else {
    sample(c("North", "West", "South", "East"), size = 1, prob = c(0.05, 0.1, 0.7, 0.15))
  }
}

pop$region_syn <- sapply(pop$onderwijs, assign_region)

assign_gender <- function(edu) {
  if (edu == "High") {
    sample(c("Man", "Vrouw"), size = 1, prob = c(0.2, 0.8))  
  } else if (edu == "Middle") {
    sample(c("Man", "Vrouw"), size = 1, prob = c(0.5, 0.5))  
  } else {
    sample(c("Man", "Vrouw"), size = 1, prob = c(0.8, 0.2))
  }
}

pop$geslacht_syn = sapply(pop$onderwijs, assign_gender)

pop$inkomen3_syn = as.factor(pop$inkomen3_syn)
pop$region_syn = as.factor(pop$region_syn)
pop$geslacht_syn = as.factor(pop$geslacht_syn)

# create the superpopulation
# log-scale the numerical variable
pop$leeftijd = scale(log(pop$leeftijd+0.0001)) 

covs = c("geslacht_syn", "leeftijd", "inkomen3_syn", "region_syn", "I(leeftijd**2)") # geslacht will be the domain variable
...
for (B in 1:B_pop) {
  ...
  theta_model[[B]] = unclass(as.matrix(
    table(sim_register$geslacht_syn, sim_register$onderwijs)))
  ...
  for (b in 1:b_samp) { # sampling variability
    ...
    # for benchmarks
    theta_hat_sampling[[b]] = unclass(as.matrix(
      table(sim_register$geslacht_syn, sim_register$onderwijs_imp)))
    difference_sampling[[b]] = theta_hat_sampling[[b]] - theta_model[[B]]
...
save(P_model, P_sampling, file = "sup2_c3_P.RData")
write.csv(results_sampling_df, "sup2_c3_sampling_distribution.csv")
write.csv(results_model_df, "sup2_c3_model_distribution.csv")
write.csv(results_joint_df, "sup2_c3_joint_distribution.csv")
...
model_paths = c(
  "sup2_baseline_model_distribution.csv",
  "sup2_small_sample_model_distribution.csv",
  "sup2_small_population_model_distribution.csv",
  "sup2_nonignorable_model_distribution.csv",
  "sup2_overparameterisation_model_distribution.csv",
  "sup2_designbias_model_distribution.csv",
  "sup2_modelbias_small_model_distribution.csv",
  "sup2_modelbias_large_model_distribution.csv",
  "sup2_modelbias_large_small_samp_model_distribution.csv"
)
...
model_distribution_sup2 = do.call(rbind, lapply(seq_along(model_paths), function(i) {
...
sampling_distribution_sup2 = do.call(rbind, lapply(seq_along(sampling_paths), function(i) {
...
joint_distribution_sup2 = do.call(rbind, lapply(seq_along(joint_paths), function(i) {
...
joint_distribution_sup2$Estimate = as.factor(joint_distribution_sup2$Estimate)
levels(joint_distribution_sup2$Estimate) = c(
...
sampling_distribution_sup2$Estimate = as.factor(sampling_distribution_sup2$Estimate)
levels(joint_distribution_sup2$Estimate) = c(
...
model_distribution_sup2$Estimate = as.factor(model_distribution_sup2$Estimate)
...
sampling_distribution_sup2_c3 = results_sampling_df
...
sampling_distribution_sup2_c3_summary = results_model_df

# save the files for analysis
save(sampling_distribution_sup2,
     model_distribution_sup2,
     joint_distribution_sup2,
     sampling_distribution_sup2_c3,
     sampling_distribution_sup2_c3_summary,
